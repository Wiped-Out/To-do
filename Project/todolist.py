...
class ToDo:
    ...
    def __init__(self):
        ...
        self.actions: dict = {'1': self.today_tasks, '2': self.week_tasks, '3': self.all_tasks, '4': self.missed_tasks,
                              '5': self.create_task, '6': self.delete_task, '0': exit}
    ...
    def all_tasks(self) -> None:
        tasks = self.session.query(Table).filter(Table.deadline).all()
        print('\nAll tasks:')
